...
from GetRandomDataBottle import GetRandomData
from CreateDatasetBottle import CreateDataset
from CombinedSSIML1LossBottle import HybridLossSSIML1Temp as combined_loss

from PeakSignalNoiseRatioBottle import calculate_psnr
from StructuralSimilarityBottle import calculate_ssim_score
...
def validation(model, device, loss_fn, loader):
    ...
    # Using inference_mode is more memory efficient than no_grad
    with torch.inference_mode():
        for lr, hr in loader:
            ...
            out = out.float()
            if out.ndim == 6:
                out = out.squeeze(2)  # Squeeze the Channel dimension (C=1)
...
if __name__ == "__main__":
    ...
    for epoch in range(NUM_EPOCHS):
        ...
        torch.cuda.empty_cache()
        avg_psnr, avg_ssim, val_loss = validation(model, DEVICE, criterion, val_loader)
        torch.cuda.empty_cache()
