...
from DownSampleBottle import DownSample
from DoubleConvolutionBottle import DoubleConv
from UpSampleBottle import UpSample
...
class UNet(nn.Module):
    def __init__(self, in_channels, out_channels, base_filters=32):
        ...
        # Encoder
        self.down_conv1 = DownSample(in_channels, f)
        self.down_conv2 = DownSample(f, f * 2)
        self.down_conv3 = DownSample(f * 2, f * 4)
        # self.down_conv4 = DownSample(f * 4, f * 8)
        ...
        # Decoder
        self.up_conv2 = UpSample(f * 4, f * 4, f * 4)
        self.up_conv3 = UpSample(f * 4, f * 2, f * 2)
        self.up_conv4 = UpSample(f * 2, f, f)
