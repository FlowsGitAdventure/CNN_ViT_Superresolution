...
# from FourierCharbonierLoss import FourierCharbonier
from lossesAniso.KSpaceGradCharLossAnisotropic import FourierCharbonier
...
SAVE_DIR = './CheckpointBaseUNetAnisotropic'
...
def train(model, device, loader, optimizer, loss_fn, epoch, num_epochs, scaler):
    ...
    for batch_idx, (lr, hr, mask, spacing) in progress_bar:
        ...
        with torch.amp.autocast(device_type=device, enabled=(device == "cuda")):
            # 1. Model predicts on the padded LR
            # Pass hr.shape so the model can perform internal alignment
            out = model(lr, hr.shape)
            out = crop_to_valid_fov(out, hr)

            loss, loss_components = loss_fn(out, hr, spacing)

            # 3. Apply Gradient Accumulation if BATCH_SIZE is small
        ...
        if (batch_idx + 1) % ACCUMULATION_STEPS == 0:
            ...
            # Clip gradients to handle potential Fourier-loss spikes
...
if __name__ == "__main__":
    # Data Setup
    data_selector = GetRandomData(LR_DIR, HR_DIR, 100, 15, is_random=True)
    ...
    sample_lr, _, _, _ = train_dataset[0]
    ...
    plt.savefig('training_resultsBaseUNetAnisotropic.png')
    plt.show()
