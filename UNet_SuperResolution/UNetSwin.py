...
class MonaiSwinSR(nn.Module):
    def __init__(self, in_channels=1, out_channels=1, base_filters=24, window_size=(4, 4, 4)):
        ...
        self.up1 = UpSample(in_channels=f * 16, out_channels=f * 8, use_norm=False, skip_channels=f * 4)

        # Level 2 to Level 1
        self.up2 = UpSample(in_channels=f * 8, out_channels=f * 4, use_norm=False, skip_channels=f * 2)

        # Level 1 to Level 0 (Patch Embed)
        self.up3 = UpSample(in_channels=f * 4, out_channels=f * 2, use_norm=False, skip_channels=f)
